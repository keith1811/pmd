...
def load_and_set_env_vars(env: str) -> Dict[str, Any]:
    ...
    # /conf/notebookdev/.gcpanman02.env
    env_vars_path = os.path.join(os.pardir, 'conf', env, f'.{env}.env')
    dotenv.load_dotenv(env_vars_path)
...
def load_config(pipeline_name, env: str) -> Dict[str, Any]:
    """
    ...
    config_path = os.path.join(os.pardir, 'conf', env, 'pipeline_configs', f'{pipeline_name}.yml')
